...
class MammothConnect:
    def __init__(self, api_key: str, api_secret: str, base_url: Optional[str] = None, 
                 workspace_id: Optional[int] = None, project_id: Optional[int] = None):
        """
    ...
    def upload_file(self, file_path: str, url: Optional[str] = None) -> requests.Response:
        """
        ...
        if url is None:
            if self.workspace_id is None or self.project_id is None:
                raise ValueError("Either provide URL or set workspace_id and project_id in constructor")
        ...
        with open(file_path, 'rb') as file:
            files = {
                'files': (file_path.name, file, content_type)
            }
